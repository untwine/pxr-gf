...
#include <pxr/gf/vec{{ DIM }}{{ SCALAR_SUFFIX(S) }}.h>
{% endfor %}
{% endif %}
                            ...
                            double eps = {{ EPS }})
{
    ...
    return pxr::boost::python::make_tuple(v1, v2);
}
...
static {{ SCL }} _SequenceGetItem(PyObject *seq, Py_ssize_t i) {
    pxr::boost::python::handle<> h(PySequence_GetItem(seq, i));
    return extract<{{ SCL }}>(pxr::boost::python::object(h));
}

static bool _SequenceCheckItem(PyObject *seq, Py_ssize_t i) {
    pxr::boost::python::handle<> h(PySequence_GetItem(seq, i));
    extract<{{ SCL }}> e((pxr::boost::python::object(h)));
    return e.check();
}
...
struct PickleSuite : pxr::boost::python::pickle_suite
{
    static pxr::boost::python::tuple getinitargs(const {{ VEC }} &v) {
        return pxr::boost::python::make_tuple({{ LIST("v[%(i)s]") }});
    }
};
