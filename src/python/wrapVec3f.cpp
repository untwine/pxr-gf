...
static float _SequenceGetItem(PyObject *seq, Py_ssize_t i) {
    ...
    return extract<float>(pxr::boost::python::object(h));
}

static bool _SequenceCheckItem(PyObject *seq, Py_ssize_t i) {
    pxr::boost::python::handle<> h(PySequence_GetItem(seq, i));
    extract<float> e((pxr::boost::python::object(h)));
    return e.check();
}
    ...
    FromPythonTuple() {
        converter::registry::
            push_back(&_convertible, &_construct,
                      pxr::boost::python::type_id<GfVec3f>());
    }
...
{
    static pxr::boost::python::tuple getinitargs(const GfVec3f &v) {
