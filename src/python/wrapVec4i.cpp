...
static int _SequenceGetItem(PyObject *seq, Py_ssize_t i) {
    ...
    return extract<int>(pxr::boost::python::object(h));
}

static bool _SequenceCheckItem(PyObject *seq, Py_ssize_t i) {
    pxr::boost::python::handle<> h(PySequence_GetItem(seq, i));
    extract<int> e((pxr::boost::python::object(h)));
    return e.check();
}
    ...
    FromPythonTuple() {
        converter::registry::
            push_back(&_convertible, &_construct,
                      pxr::boost::python::type_id<GfVec4i>());
    }
...
{
    static pxr::boost::python::tuple getinitargs(const GfVec4i &v) {
