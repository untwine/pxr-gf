...
static GfHalf _SequenceGetItem(PyObject *seq, Py_ssize_t i) {
    ...
    return extract<GfHalf>(pxr::boost::python::object(h));
}

static bool _SequenceCheckItem(PyObject *seq, Py_ssize_t i) {
    pxr::boost::python::handle<> h(PySequence_GetItem(seq, i));
    extract<GfHalf> e((pxr::boost::python::object(h)));
    return e.check();
}
    ...
    FromPythonTuple() {
        converter::registry::
            push_back(&_convertible, &_construct,
                      pxr::boost::python::type_id<GfVec3h>());
    }
...
{
    static pxr::boost::python::tuple getinitargs(const GfVec3h &v) {
        return pxr::boost::python::make_tuple(v[0], v[1], v[2]);
    }
};
